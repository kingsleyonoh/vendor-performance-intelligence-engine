...
module Ecosystem
  # Faraday 2 client to the Transaction Reconciliation Engine
  # (PRD §6 + §13.2). Mirrors `HubClient` / `WorkflowClient` /
  # `WebhookEngineClient` / `InvoiceReconClient` / `ContractEngineClient`
  # — same Faraday + retry + circuit-breaker pattern per
  # `.claude/knowledge/foundation/ecosystem-client-pattern.md`.
  #
  # Singleton wired in `config/initializers/ecosystem_clients.rb`.
  # Standalone-first: when `RECON_ENGINE_ENABLED != "true"`, every
  # public method returns `{status: :skipped, ...}` immediately without
  # HTTP (PRD §2.2).
  #
  # Public surface (PRD §6 — Transaction Recon adapter):
  #   - list_discrepancies(source:, since:, page:)      → GET /api/v1/discrepancies?source=vendor
  #   - fetch_stats(vendor_ref:, since:, until_time:)   → GET /api/v1/stats
  class ReconEngineClient
    DEFAULT_BASE_URL = "http://recon-engine.example.test"
    OPEN_TIMEOUT     = 5
    READ_TIMEOUT     = 30
    ...
    def initialize(adapter: nil, breaker: nil, base_url: nil)
      @base_url   = base_url || ENV.fetch("RECON_ENGINE_URL", DEFAULT_BASE_URL)
      @api_key    = ENV.fetch("RECON_ENGINE_API_KEY", "test-key")
      @breaker    = breaker || CircuitBreaker.new
      @connection = build_connection(adapter: adapter)
    end

    # GET /api/v1/discrepancies?source=vendor → reconciliation discrepancy
    # records over a window. PRD §6b pins the contract to source=vendor.
    def list_discrepancies(source: "vendor", since: nil, page: 1)
      return skipped_response unless enabled?

      params = build_window_params(since: since, until_time: nil)
      params[:source] = source || "vendor"
      params[:page] = page if page

      get_json("/api/v1/discrepancies", params: params) do |body|
        {
          discrepancies: extract_array(body, "discrepancies"),
    ...
    # GET /api/v1/stats → vendor-scoped aggregate transactional-signal stats.
    def fetch_stats(vendor_ref:, since: nil, until_time: nil)
      ...
      get_json("/api/v1/stats", params: params) do |body|
        { stats: body.is_a?(Hash) ? body : {} }
      end
    end

    def enabled?
      ENV.fetch("RECON_ENGINE_ENABLED", "false").to_s.downcase == "true"
    end
    ...
    def skipped_response
      { status: :skipped, reason: "Recon Engine disabled" }
    end
    ...
    def get_json(path, params: {})
      @breaker.call do
        begin
          ...
          raise TransientFailure, "Recon Engine retry exhausted (#{status || 'unknown'})"
        rescue *RETRY_EXCEPTIONS => e
          raise TransientFailure, "Recon Engine network failure: #{e.class}: #{e.message}"
        end
        ...
        if response.status.between?(200, 299)
          ...
          raise TransientFailure, "Recon Engine returned #{response.status}"
        else
    ...
    def extract_error_message(response)
      body = parse_body(response)
      if body.is_a?(Hash)
        body["error"] || body["message"] || body["detail"] || "Recon Engine returned #{response.status}"
      else
        "Recon Engine returned #{response.status}"
      end
    end
    ...
    def build_connection(adapter:)
      Faraday.new(url: @base_url) do |f|
        f.request :json
        f.response :logger, Rails.logger.tagged("ecosystem.recon_engine"), bodies: false do |logger|
