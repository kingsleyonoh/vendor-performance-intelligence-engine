...
module Ingestion
  ...
  #
  # Two trigger paths:
  #   1. Sidekiq cron — every 15 min, iterates over enabled invoice_recon
  ...
  #     `Ecosystem::InvoiceReconClient.fetch_stats(vendor_ref:, since:, until_time:)`.
  #   - Map stats → signal payloads via `Ingestion::Mappers::InvoiceReconMapper`.
  ...
  class InvoiceReconBackfillJob < ApplicationJob
    queue_as :default

    # Cron-mode entry: no args → iterate enabled invoice_recon sources for
    ...
    def cron_iterate
      IngestionSource
        .where(source_system: "invoice_recon", is_enabled: true)
        .find_each do |source|
          self.class.perform_later(source.id)
        rescue StandardError => e
          Rails.logger.error("[invoice_recon_pull] failed to enqueue source=#{source.id}: #{e.class}: #{e.message}")
        end
    end
    ...
    def run_for_source(source_id, ingestion_run_id)
      ...
      unless source.is_enabled
        Rails.logger.tagged("ingestion.invoice_recon") do
    ...
    def execute_pull!(source:, run:)
      ...
      vendors_for(source).each do |vendor|
        ...
        when :skipped
          # Standalone-first short-circuit — first vendor's :skipped
          # means the integration is feature-flagged off. Stop iterating
          # and treat the run as a successful no-op.
          mark_succeeded!(run, source: source, cursor: until_time,
                          attempted: 0, stored: 0, rejected: 0, deduped: 0)
          return run
        when :failed
          had_failure = true
          mark_failed!(run, error: "Invoice Recon returned #{response[:response_code]}: #{response[:error]}")
          return run
        when :ok
    ...
    def ingest_stats!(run:, source:, vendor:, stats:, recorded_at:)
      payloads = Ingestion::Mappers::InvoiceReconMapper.map_stats(
      ...
      payloads.each do |payload|
        ...
        rejected += 1
        Rails.logger.error("[invoice_recon_pull] ingest error: #{e.class}: #{e.message}")
      end

      # Accumulate counters on the run row.
      run.update_columns(
